...
#nonsym data
DO.size <- DO.gpa$Csize[names(DO.gpa$Csize) %in% dimnames(DO.probs)[[1]]]
#sym data
...
combined.markers <- read.csv("/mnt/Hallgrimsson/Users/Katz/DO_Mice/QTL_analysis/rqtl2_lite/data/qtl2_files/MMnGM_processed_files/MMnGM_info.csv", comment.char="#")
...
DO.resids <- two.d.array(sweep(x = DO.pheno, MARGIN = 2,STATS = apply(DO.pheno, 2, mean),FUN = "-"))
...
ultimate_theme <-   theme(
        ...
        axis.text = element_text(angle = 55, hjust = 1, size = 16),
...
png("fgf_gloadings.png", height = 750, width = 1100)
p
dev.off()
...
f <- c(1,19,32,50,40,9,8, 39,47,16, 20:23, 12,17,18,51:54, 43,49,48,11,42,10,24,41,31)
v <- c(13,44,25,26)
b <- c(3,34,7,38,35,28,4,2,33,14,15,45,46,36,5,6,27,37)

spheres3d(fixed.do.mean[f,], radius = .004, color = adjustcolor("red", .3))
spheres3d(fixed.do.mean[v,], radius = .004, color = adjustcolor("green", .3))
spheres3d(fixed.do.mean[b,], radius = .004, color = adjustcolor("blue", .3))
...
shape.warp <-  plot3d(shape.mean$mesh, col = adjustcolor("lightgrey", .7), alpha = .4, specular = 1, axes = F, box = F, xlab = "", ylab = "", zlab = "", main = "")
# text3d(fixed.do.mean, texts = 1:54)
f <- c(1,19,32,50,40,9,8, 39,47,16, 20:23, 12,17,18,51:54, 43,49,48,11,42,10,24,41,31)
v <- c(13,44,25,26)
b <- c(3,34,7,38,35,28,4,2,33,14,15,45,46,36,5,6,27,37)

spheres3d(fixed.do.mean[,], radius = .004, color = adjustcolor("red", .3))
...
```{r calc kinship based on correlation}
library(ggplot2)
load("~/Documents/MGP_epistasis/2state_epistasis.Rdata")
...
ggplot() +
  geom_histogram(aes(as.numeric(K))) + 
  xlim(-.1, 1) +
  xlab("Pairwise genetic kinship value (K)")

boxplot(as.numeric(K))


```


```{r sparse process MGP: FGF with kinship}
library(geomorph)
library(MASS)
library(ddsPLS)
library(ggplot2)
...
Kinv <- ginv(DO.K)
glsX <- t(probs.rows) %*% Kinv

process.svd <- mddsPLS(Xs = t(glsX), Y = Y, R = 1, lambda = .08)
process.svd.test <- mddsPLS(Xs = probs.rows, Y = Y, R = 1, lambda = .06)
test <- pls2B(t(glsX),  Y)
...
pathway.loadings <- data.frame(gloadings = test$svd$u[,1], gnames = sort(as.character(rep(seq.info[,1], each = 8))), founders = rep(do.names, nrow(seq.info)))
...
# proj.coords.a1 <- row2array3d(rbind(which.min(test$Yscores[,1]) * test$svd$v[,1] + test$ycenter, which.max(test$Yscores[,1]) * test$svd$v[,1] + test$ycenter), Nlandmarks = 54)
# proj.coords.a2 <- proj.coords.a1[,,2]
# proj.coords.a1 <- proj.coords.a1[,,1]
...
png("chond_gloadings.png", height = 750, width = 1100)
p
dev.off()
...
#map of centimorgan distances, so we can permute random MGP markers with similar patterns of LD
#for each chromosome, save the cm distances between all markers
unique(combined.markers.ordered$chr)
plot(diff(c(0, combined.markers.ordered$cM_cox[combined.markers.ordered$chr == 1])))

marker.dists <- NULL

for(i in 1:length(unique(combined.markers.ordered$chr))) marker.dists <- c(marker.dists, diff(c(0, combined.markers.ordered$cM_cox[combined.markers.ordered$chr == unique(combined.markers.ordered$chr)[i]])))

#in the MGP_permuter function, choose the markers with the closest cm diffs from the real thing
# need to know the index position of chosen marker per gene
...
```{r random marker selection for permutation with LD}

#map of centimorgan distances, so we can permute random MGP markers with similar patterns of LD
#for each chromosome, save the cm distances between all markers

combined.markers.ordered <- NULL
for(i in 1:length(unique(combined.markers$chr))){
  
  combined.markers.ordered <- rbind(combined.markers.ordered, combined.markers[combined.markers$chr == unique(combined.markers$chr)[i], ][sort(combined.markers$Mbp_mm10[combined.markers$chr == unique(combined.markers$chr)[i]], index.return = T)$ix, ])
...
for(j in 1:n_perms){
  #pick markers with similar ld
  permuted.markers <- matrix(NA, nrow = n_perms, ncol = nrow(seq.indexes))
  for(k in 1:nrow(seq.indexes)){
  marker.cm <- marker.dists[combined.markers.ordered$marker == seq.indexes[k,2]]
  permuted.markers[,k] <- sort(abs(marker.dists - marker.cm), index.return = T)$ix[1:nperm]
  
  }
  

 probs.rows <- cbind(probs.rows, DO.probs[,,permuted.markers[j,]])
...
load("random_process_eigensimilarity.Rdata")
giga.pca <- prcomp(Y)
...
process.correlations3 <- rep(NA, ncol(process.loadings))
process.correlations4 <- rep(NA, ncol(process.loadings))
...
for(i in 1:ncol(process.loadings)){
  ...
  process.correlations2[i] <- cor(giga.pca$rotation[,2], process.loadings[,i])
  process.correlations3[i] <- cor(giga.pca$rotation[,3], process.loadings[,i])
  process.correlations4[i] <- cor(giga.pca$rotation[,4], process.loadings[,i])
...
PC.corrs <- data.frame(process.name = process.name, cor1 = abs(process.correlations), cor2 = abs(process.correlations2), cor3 = abs(process.correlations3), cor4 = abs(process.correlations4))

View(PC.corrs[sort(PC.corrs[,2], index.return = T)$ix,])
...
process.cor.df <- data.frame(process.name = as.character(process.name), cor = abs(process.correlations), cor2 = abs(process.correlations2), caccor = abs(cac.cor))

process.cor.df[,1] <- as.character(process.cor.df[,1])

ggplot(process.cor.df, aes(x = cor, fill = "#E69F00", text = process.name)) + 
  geom_histogram(alpha = 1, show.legend = FALSE) + 
  xlab("Correlation to PC1") +
  ylab("Frequency") +
  xlim(range(process.cor.df$cor)) +
  scale_fill_manual(values = "#E69F00") +
  theme_bw()

ggplot(process.cor.df, aes(x = cor2, fill = "#E69F00", text = process.name)) + 
  geom_histogram(alpha = 1, show.legend = FALSE) + 
  xlab("Correlation to PC2") +
  ylab("Frequency") +
  scale_fill_manual(values = "#E69F00") +
  theme_bw()

ggplot(process.cor.df, aes(x = caccor, fill = "#E69F00", text = process.name)) + 
  geom_histogram(alpha = 1, show.legend = FALSE) + 
  xlab("Correlation to CAC") +
  ylab("Frequency") +
  scale_fill_manual(values = "#E69F00") +
  theme_bw()
...
#all 4 PCs as densities
plotfill <- RColorBrewer::brewer.pal(12, "RdGy")
ggplot(PC.corrs) +
  geom_density(aes(x = cor1, fill = "PC1"), alpha = .7, show.legend = T) + 
  geom_density(aes(x = cor2, fill = "PC2"), alpha = .7) +
  geom_density(aes(x = cor3, fill = "PC3"), alpha = .7) + 
  geom_density(aes(x = cor4, fill = "PC4"), alpha = .7) + 
...
mutant.mean.df <- data.frame(mutant = c(mutant.mean.df[,1], proc.score[,1]), scores = rbind(as.matrix(mutant.mean.df[,-1]), cbind(as.numeric(proc.score[,2]), as.numeric(proc.score[,3]))), Effect = c(rep("Mutant", 36), rep("Genomic", 8)))
mutant.mean.df[,1] <- as.character(mutant.mean.df[,1])
...
mutant.mean.df[35,1] <- "Shh"
mutant.mean.df[29,1] <- "Spry1_hom"
mutant.mean.df[30,1] <- "Spry1_het"
mutant.mean.df[31,1] <- "Spry2_hom"
mutant.mean.df[32,1] <- "Spry2_het"
mutant.mean.df[33,1] <- "Spry4_hom"
mutant.mean.df[34,1] <- "Spry4_het"
...
colnames(proc.score) <- c("process", "score1", "score2")
 ...
 png("~/Downloads/morphospace_DOMGP2.png", width = 550*1.5, height = 350*1.5)  
...
for(k in 11:length(tree.processes)){
...
pathway.loadings <- data.frame(gloadings = pls.svd$mod$u[[1]], gnames = sort(as.character(rep(seq.info[,1], each = 8))), founders = rep(do.names, nrow(seq.info)))
...
```{r cophenetic correlation: MGP vectors}
coph.tree <- heatmap(1-tree.cor, labRow = tree.processes, labCol = tree.processes, symm = T, keep.dendro = T)

res <- dist(cophenetic(coph.tree$Rowv))
res <- as.matrix(res)
res <- as.dist(res[order(rownames(res)),order(colnames(res))])


res2 <- dist(1-tree.cor)
res2 <- as.matrix(res2)
res2 <- as.dist(res2[order(rownames(res2)),order(colnames(res2))])
...
p.loadings <- process.loadings[, !apply(is.na(process.loadings), 2, all)]
p.names <- process.name[!apply(is.na(process.loadings), 2, all)]

tree.size <- c()#sample(1:967, 50)

# tree.cor <- abs(cor(p.loadings[,tree.size]))

# colnames(tree.cor) <- p.names[tree.size]
# rownames(tree.cor) <- p.names[tree.size]

# heatmaply::heatmaply(tree.cor, symm = T, showticklabels = c(F,F))

#what do we want to do? Compare a few mutants to random directions in process space
p.loadings[,tree.size]
mutant.names <- as.character(unique(mutant.dbfin$Genotype))
mutant.names[5] <- "B9D1"
mutant.names[7] <- "Bmp2_7"
mutant.names[9] <- "Bmp7 cond"
mutant.names[10] <- "Crf4"
mutant.names[11] <- "Disp"
mutant.names[12] <- "Fgf10"
mutant.names[13] <- "Fgf3"
mutant.names[14] <- "Ghrhr"
mutant.names[16] <- "Grm1 cond"
mutant.names[17] <- "Hhat"
mutant.names[18] <- "Ift88"
mutant.names[19] <- "Lrp2" #Lrp2
mutant.names[20] <- "Mceph"
mutant.names[22] <- "Nipbl"
mutant.names[24] <- "Nosip hom"
mutant.names[25] <- "Nosip het" #Nosip het
mutant.names[26] <- "Papps2"
mutant.names[27] <- "Pten het"
mutant.names[28] <- "Pten hom"
mutant.names[29] <- "Shh"
mutant.names[8] <- "Bmp7" #Bmp7


#we need loadings for each direction in mutant.dbfin$Genotype
mutant.loadings <- matrix(NA, nrow = 54*3, ncol = length(unique(mutant.dbfin$Genotype))) 

for(i in 1:ncol(mutant.loadings)){
tmp.mutant.registration <- gpagen(arrayspecs(rbind(Y, as.matrix(mutant.lmsfin[mutant.dbfin$Genotype == unique(mutant.dbfin$Genotype)[i],])), 54, 3))$coords
    
    do.mean <- array.mean(tmp.mutant.registration[,,1:nrow(Y)])
    mutant.mean <- array.mean(tmp.mutant.registration[,,-(1:nrow(Y))])

mutant.loadings[,i] <- manova(two.d.array(tmp.mutant.registration) ~ c(rep(0, nrow(Y)), rep(1, sum(mutant.dbfin$Genotype == unique(mutant.dbfin$Genotype)[i]))))$coef[2,]
}

#make a correlation matrix between the loadings of each process and mutant
tree.size <- c(14, 15, 18, 24, 31, 33, 35, 39, 42, 51, 52, 57, 61, 62, 75, 109, 145, 234, 252, 313, 347, 358, 397, 410, 566, 569, 618, 735, 880)#sample(1:967, 50)
tree.cor <- cor(cbind(p.loadings[,tree.size], mutant.loadings))[(length(tree.size) + 1):(length(tree.size) + 30),1:length(tree.size)]

colnames(tree.cor) <- p.names[tree.size]
rownames(tree.cor) <- mutant.names

heatmaply::heatmaply(abs(tree.cor), symm = F, showticklabels = c(T,T), plot_method = "ggplot", cexRow = .75, cexCol = .75, margins = c(250,150,NA,0), xlab = "MGP effects", ylab = "Mutant effects")
...
#df contains the genome for one ind with cols for mb, csome, founder1, founder2

do.ex <- DO.probs[900,,]

mb <- combined.markers$Mbp_mm10
chr <- as.numeric(combined.markers$chr)
founder1 <- rep(NA, length(chr))
founder2 <- rep(NA, length(chr))
c1 <- rep(NA, length(chr))
c2 <- rep(NA, length(chr))

for(i in 1: length(c1)){
  if(sum(round(do.ex[,i], 2) > .8) == 1){
    founder1[i] <- do.names[which(round(do.ex[,i], 2) > .8)[1]]
  founder2[i] <- do.names[which(round(do.ex[,i], 2) > .8)[1]]
  c1[i] <- do.colors[which(round(do.ex[,i], 2) > .8)[1]]
  c2[i] <- do.colors[which(round(do.ex[,i], 2) > .8)[1]]
    
  } else {
  founder1[i] <- do.names[which(round(do.ex[,i], 2) > .4)[1]]
  founder2[i] <- do.names[which(round(do.ex[,i], 2) > .4)[2]]
  c1[i] <- do.colors[which(round(do.ex[,i], 2) > .4)[1]]
  c2[i] <- do.colors[which(round(do.ex[,i], 2) > .4)[2]]
  }
  }

geno.df.1 <- data.frame(mb, chr - .2, founder = founder1, group = rep("hap1", length(chr)), c = c1)
geno.df.2 <- data.frame(mb, chr + .2, founder = founder2, group = rep("hap2", length(chr)), c = c2)

geno.df <- rbind.data.frame(geno.df.1, geno.df.2)

# ggplot(geno.df) +
#   geom_point(aes(x = chr, y = mb, fill = c), stat = "identity", position = "dodge") +
#   scale_fill_manual(values = do.colors)

plot(geno.df$chr ~ geno.df$mb, col = as.character(geno.df$c), pch = 15, cex = .5, axes = F, xlab = "", ylab = "", ylim = c(1,25))
legend(x = 20, y = 24, legend = do.names, col = do.colors, pch = 15, horiz = T, cex = .35, pt.cex = .8)

```

```{r sparse process MGP: zinc ion binding}

#instead of process.ano doing pattern matching, we need to use it to match GO terms precisely
selection.vector <- c('zinc ion binding')
...
# png("chond_gloadings.png", height = 500, width = 750)
png("zinc_ion_binding_gloadings.png", height = 750, width = 7000)
p
dev.off()
