...
proc.score[k,1] <- process.name
  ...
  #try it in ggplot
  mutant.mean.df <- data.frame(mutant = c(as.character(unique(mutant.dbfin$Genotype)), "DO"), scores = rbind(mutant.mean.pc, do.mean.proj[1:2]))
...
colnames(proc.score) <- c("process", "score1", "score2")
  ...
  ggplot(mutant.mean.df, aes(x = scores.1, y = scores.2, label = mutant)) +
...
ggrepel::geom_text_repel(segment.alpha = .2, size = 2.25, max.iter = 5000) +
theme_bw()
